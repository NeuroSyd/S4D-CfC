# Code created by Zhaojing Huang for training the model on 05 December 2023
...
# Optimizer
parser.add_argument('--file_name', default='S4D_NCP_test1_normalized', type=str, help='Folder Name')
...
with h5py.File('/x.hdf5', 'r') as f:
    X = f['tracings'][:, :, 1].reshape(-1, 4096, 1) # [:]  # shape (B, 4096, 12)
    y = pd.read_csv('/y.csv').values.reshape(-1, 6)  # shape (B, 8)
...
csv_file = directory_path + '/results.csv'
...
torch.save(model.state_dict(), directory_path + '/brazil_model.pt')
